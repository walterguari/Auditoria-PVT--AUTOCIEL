...
def cargar_todo(url, proceso):
    try:
        ...
        # Estructura compartida: Col F(5)=Pregunta, Col G(6)=Descripción, Col L(11)=Score
        ...
        st.error(f"Error al conectar con la base de datos de {proceso}.")
...
if not st.session_state.auditoria_activa:
    ...
    if total > 0:
        st.markdown("### 📅 Tendencia de Resultados")
    ...
    if st.button(f"🚀 Iniciar Nueva Auditoría de {st.session_state.proceso_seleccionado}", use_container_width=True, type="primary"):
...
# --- PANTALLA 2: FORMULARIO ---
    ...
    st.title(f"📝 Realizando Auditoría: {st.session_state.proceso_seleccionado}")
    ...
    if c_x.button("⬅️ Volver al Dashboard"):
        st.session_state.auditoria_activa = False
        st.rerun()
    ...
    with st.container(border=True):
        ...
        # Selectores personalizados según el área
        if st.session_state.proceso_seleccionado == "CCS":
            asesor_n = f3.selectbox("Asesor Auditado", ["Haydeé", "Antonio"])
        elif st.session_state.proceso_seleccionado == "CITAS":
            # Aquí puedes poner los nombres de los encargados de Citas
            operador_n = f3.selectbox("Operador Auditado", ["Gustavo", "Dani", "Otro"])
        else:
            f3.empty()
    ...
    for i, preg in enumerate(lista_preguntas):
        with st.expander(f"{i+1}. {preg}", expanded=resp_actuales[i] in ["Pendiente", "No Cumple"]):
            ...
            archivos = c_f.file_uploader(f"Evidencia (Máx 6 fotos)", type=['jpg','png','jpeg'], accept_multiple_files=True, key=f"f_{i}")
    ...
    if st.button("💾 Finalizar y Guardar Auditoría", use_container_width=True, type="primary"):
        if contestadas < len(lista_preguntas) or not auditor_n:
            st.warning("⚠️ Asegúrese de responder todas las preguntas e ingresar su nombre.")
        else:
