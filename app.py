...
if 'proceso_seleccionado' not in st.session_state:
    ...
    st.subheader("¿Qué proceso desea auditar hoy?")
...
def cargar_todo(url, proceso):
    ...
    # Adaptación de columnas según el proceso
    if proceso == "GESTION":
        col_preg, col_desc, col_score = 5, 6, 11
    else: # Estructura CCS
        col_preg, col_desc, col_score = 5, 6, 11 # Ajustar si CCS varía
        
    df_preg = df.iloc[:, [col_preg, col_desc]].dropna(subset=[df.columns[col_preg]])
    mapa_desc = dict(zip(df_preg.iloc[:, 0], df_preg.iloc[:, 1]))
    lista_preg = list(mapa_desc.keys())
    
    df_hist = df.copy()
    df_hist.iloc[:, col_score] = pd.to_numeric(df_hist.iloc[:, col_score], errors='coerce')
    historial = df_hist[df_hist.iloc[:, col_score].notnull()].reset_index(drop=True)
...
try:
    ...
    if not st.session_state.auditoria_activa:
        ...
        # Filtros y Métricas (Mantenemos la lógica que ya funcionaba)
        ...
        if st.button(f"🚀 Iniciar Auditoría de {st.session_state.proceso_seleccionado}", use_container_width=True, type="primary"):
    ...
    # --- PANTALLA 2: FORMULARIO ---
        ...
        st.title(f"📝 Nueva Auditoría: {st.session_state.proceso_seleccionado}")
        ...
        with st.container(border=True):
            ...
            # Para CCS, agregamos selector de Asesor
            if st.session_state.proceso_seleccionado == "CCS":
                asesor = f3.selectbox("Asesor Auditado", ["Haydeé", "Antonio"])
            else:
                f3.write("")

        datos_adicionales = {}
        for i, pregunta in enumerate(lista_preguntas):
            with st.expander(f"{i+1}. {pregunta}", expanded=resp_actuales[i] in ["Pendiente", "No Cumple"]):
        ...
        if st.button("💾 Finalizar y Guardar Todo", use_container_width=True, type="primary"):
            if contestadas < len(lista_preguntas) or not auditor_n:
                ...
                with st.spinner("Guardando en la base de datos seleccionada..."):
                    id_audit = f"AUD-{len(df_historial)+1}"
                    # Guardamos fila nueva
                    nueva_fila = pd.DataFrame([[str(fecha_a), auditor_n, id_audit, "", "", "", "", "", "", str(datos_adicionales), str(list(resp_actuales.values())), score_vivo]], columns=df_base.columns)
