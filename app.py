...
# Función para cargar datos (Preguntas y Historial)
...
try:
    ...
    if not st.session_state.auditoria_activa:
        st.subheader("📊 Cumplimiento Mensual de Gestión")
        
        if not df_historico.empty and df_historico.shape[1] > 6:
            df_plot = df_historico.copy()
            # Convertimos Columna A a fecha y G a categoría
            ...
            if not df_plot.empty:
                ...
                # Calculamos cumplimiento por mes
                ...
                fig.update_layout(
                    xaxis_title="Mes de Auditoría", yaxis_title="% Score",
                    yaxis=dict(range=[0, 110]), template='plotly_white', height=450
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Inicia una auditoría para comenzar a visualizar los resultados mensuales.")
    ...
    # --- PANTALLA 2: CHECKLIST ---
        ...
        score_actual = (si / validas) * 100 if validas > 0 else 0

        p_avance.metric("Progreso", f"{int(avance)}%")
        p_avance.progress(avance / 100)
        p_score.metric("Cumplimiento", f"{int(score_actual)}%", delta=f"{int(score_actual-90)}% vs Obj")

        if st.button("💾 Finalizar y Guardar Auditoría", use_container_width=True):
            if avance < 100:
                st.warning("Debe completar todas las preguntas del checklist.")
            else:
                with st.spinner("Actualizando planilla..."):
                    # Actualizamos el historial con la fecha y las respuestas
                    ...
                    st.cache_data.clear() # Limpiar caché para actualizar el gráfico
